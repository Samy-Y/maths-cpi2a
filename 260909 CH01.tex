...
\begin{proof}
    En posant $u : \Kset[X] \to \Kset[a]$ avec $P \mapsto P(a)$ un isomorphisme, on a $u(P) = P(u)$ dans $\Kset[a]$ si et seulement si $P$ est inversible dans $\Kset[X]$. Or, les seuls polynômes inversibles dans $\Kset[X]$ sont les polynômes constants non nuls $\lambda \in \Kset^*$. Donc $P(a) = \lambda 1_E$ avec $\lambda \in \Kset^*$, ce qui prouve la propriété.
\end{proof}
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \item \textbf{1 $\implies$ 3 :} Supposons que $P(a)$ est inversible dans $E$.
        
        On pose $D = P \wedge \pi_a$. Alors $D \mid P$ et $D \mid \pi_a$, donc il existe $P_1, P_2 \in \Kset[X]$ tels que $P = DP_1$ et $\pi_a = DP_2$ avec $P_1 \wedge P_2 = 1$.
        
        En évaluant en $a$, on obtient $P(a) = D(a)P_1(a)$ et comme $P(a)$ est inversible dans $E$ alors $\exists y \in E$ tel que $P(a)y = 1_E$, donc $D(a)P_1(a)y = 1_E$ donc $P_2(a)D(a)P_1(a)y = P_2(a)$.
        
        Or $\pi_a = P_2D$ donc $\pi_a(a) = 0$ et donc $P_2(a)D(a)P_1(a)y = 0$. On obtient donc $P_2(a) = 0$, donc $\pi_a \mid P_2$ et donc $DP_2 \mid P_2$ et donc $D = 1$ sachant que $D$ est unitaire. Donc $P \wedge \pi_a = 1$.
    \end{itemize}
\end{proof}
\todo
% A REVOIR !!
...
\begin{example}
    ...
    Soit $A = \left(\begin{matrix} \alpha & \cdots & \cdots & \cdots & \cdots \\ \vdots & \ddots & (1) & \cdots & \vdots \\ \vdots & (1) & \ddots & (1) & \vdots \\ \vdots & \cdots & (1) & \ddots & \vdots \\ \cdots & \cdots & \cdots & \cdots & \alpha\end{matrix}\right)$ avec $\alpha \in \R$.

    Déterminons quelles valeurs $\alpha$ de $\R$ permettent à $M$ d'être inversible, et donnons son inverse.

    % On peut écrire :
    % $$M = \left(\begin{matrix} 1 & \cdots & \cdots \\ \vdots & \ddots & (1) \\ \vdots & (1) & \ddots \\ \vdots & \cdots & (1) \\ \cdots & \cdots & 1\end{matrix}\right) + (\alpha - 1)I_n = A + (\alpha - 1)I_n$$

    % Et on sait déjà que $\pi_A = X^2 - nX$ est le polynôme minimal de $A$. Donc $\pi_M(X) = \pi_{A + (\alpha - 1)I_n}(X) = \pi_A(X - (\alpha - 1)) = (X - (\alpha - 1))^2 - n(X - (\alpha - 1))$.
...
\begin{correction}
    Comme $T_n^s(\Kset)$ forme une sous-algèbre de $\mathcal M_n(\Kset)$, $\Kset[A] \subset T_n^s(\Kset)$. Comme $A$ est inversible, alors $A^{-1} \in \Kset[A] \subset T_n^s(\Kset)$, donc $A^{-1}$ est triangulaire supérieure.
\end{correction}
...
\begin{customenv}{\faIcon{sticky-note}}{Note}
    Même dans un espace vectoriel de dimension infinie, on peut trouver des endomorphismes qui admettent un polynôme minimal. Par exemple, les projections ; qui vérifient $u^2 = u$ et donc $\pi_u(X) \mid X^2 - X$.
\end{customenv}
